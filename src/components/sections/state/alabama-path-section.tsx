...
type StatePathSectionProps = {
  content: StatePageContent;
};

export function StatePathSection({ content }: StatePathSectionProps) {
  const { path } = content;

  return (
    <section id="path" className="state-bento-section scroll-mt-32 bg-paper py-16 sm:py-24">
      <SectionContainer>
        ...
        <div className="mt-10 grid items-start gap-8 lg:grid-cols-12 lg:gap-10">
          <Reveal className="lg:col-span-5" direction="left" distance={24} delay={0.05}>
            <div className="state-price-panel h-full p-7 sm:p-9">
          ...
          <Reveal className="lg:col-span-7" direction="right" distance={20} delay={0.1}>
            <div className="state-path-steps">
              {path.steps.map((step, index) => (
                <div key={step.id} className="state-path-step">
                  <div className="state-path-step__marker" aria-hidden>
                    <span className="state-path-step__num">{step.id}</span>
